...
static void dfd_set_big_cluster_rstcon(void)
{
	...
	reg_val = readl(EXYNOS9630_POWER_BASE + CPU_RESET_DISABLE_SOFTRESET);
	if (reg_val & (PEND_BIG | PEND_LITTLE)) {
		reg_val &= ~(PEND_BIG | PEND_LITTLE);
		writel(reg_val, EXYNOS9630_POWER_BASE + CPU_RESET_DISABLE_SOFTRESET);
	}
	reg_val = readl(EXYNOS9630_POWER_BASE + CPU_RESET_DISABLE_WDTRESET);
	if (reg_val & (PEND_BIG | PEND_LITTLE)) {
		reg_val &= ~(PEND_BIG | PEND_LITTLE);
		writel(reg_val, EXYNOS9630_POWER_BASE + CPU_RESET_DISABLE_WDTRESET);
	}

	/* reset enable for BIG */
	for (val = 0; val < NR_BIG_CPUS; val++) {
		reg_val = readl(EXYNOS9630_POWER_BASE + BIG_CPU0_RESET + val * 0x80);
		reg_val &= ~(RESET_DISABLE_WDT_CPUPORESET);
		reg_val &= ~(RESET_DISABLE_CORERESET | RESET_DISABLE_CPUPORESET);
		writel(reg_val, EXYNOS9630_POWER_BASE + BIG_CPU0_RESET + val * 0x80);
	}
	reg_val = readl(EXYNOS9630_POWER_BASE + BIG_NONCPU_RESET);
	reg_val &= ~(RESET_DISABLE_L2RESET | RESET_DISABLE_WDT_L2RESET);
	reg_val &= ~(RESET_DISABLE_WDT_PRESET_DBG | RESET_DISABLE_PRESET_DBG);
	writel(reg_val, EXYNOS9630_POWER_BASE + BIG_NONCPU_RESET);
}
...
void dfd_set_dump_gpr(int en)
{
	...
	writel(val, EXYNOS9630_POWER_RESET_SEQUENCER_CONFIGURATION);
	val = readl(EXYNOS9630_POWER_RESET_SEQUENCER_CONFIGURATION);
...
{
	u32 val = readl(EXYNOS9630_POWER_BASE + TIMEOUT_RESET_LPI);
...
void dfd_run_dump_gpr(void)
{
	...
	reg = readl(EXYNOS9630_POWER_RESET_SEQUENCER_CONFIGURATION);
