...
static u32 gpt_get_offset_in_secs(PART_ENTRY *part_e, u64 _offset_in_bytes, u32 block_in_secs, u32 block_in_bytes)
{
	...
	if (_offset_in_bytes) {
		if (_offset_in_bytes % SECTOR_SIZE) {
			...
			offset_in_secs = (u32)((start_in_bytes + _offset_in_bytes) / SECTOR_SIZE);
	} else
		offset_in_secs = (u32)(gpt_e->part_start_lba * block_in_secs);
...
static u32 gpt_get_size_in_secs(PART_ENTRY *part_e, u64 offset_in_bytes, u64 _size_in_bytes, u32 block_in_secs)
{
	 ...
	 */
	gpt_e = (struct gpt_entry *)part_e->gpt_entry;
	size_in_bytes_b = size_in_bytes = (gpt_e->part_end_lba - gpt_e->part_start_lba + 1) * s_block_in_bytes;

	if (_size_in_bytes) {
		if (_size_in_bytes % SECTOR_SIZE) {
		...
		PART_ENTRY *part_e, void* buf, u64 _offset_in_bytes, u64 _size_in_bytes)
{
	...
	/* Raw access case */
	secs = gpt_access_disk(buf, op, part_e->name, part_e->lun, offset_in_secs, size_in_secs);
	if (secs == 0)
		res = -1;
end:
	return res;
